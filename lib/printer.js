...
function build(ast, lastLoc, options) {
  ...
  switch (ast.type) {
    ...
    case 'BooleanLiteral':
      ...
      break;
    case 'BlockStatement': {
      const { output: open, lastLoc: openLoc } = openBlock(ast, lastLoc, options);
      output.push(...open);
      let loc = openLoc;
      ...
      const { output: program, lastLoc: progLoc } = build(ast.program, lastLoc, options);
      loc = progLoc;
      output.push(...program);
      ...
      if (ast.inverse) {
        ...
        const { output: inverse, lastLoc: inverseLoc } = build(ast.inverse, ast.inverse.loc, options);
        output.push(...inverse);
        loc = inverseLoc;
      }
...
function closeBlock(block, lastLoc, options) {
  lastLoc = locAppend(lastLoc.start, { column: 3 }); // {{/
  const { output: path, lastLoc: paramsLoc } = build(block.path, lastLoc, options);
  return ['{{/', ...path, '}}'];
}
